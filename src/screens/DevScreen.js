...
export default DevScreen = ({ navigation, route }) => {
  ...
  const encode = async () => {
    const pipelines = [
      new Pipeline(1, 'sdsdsds', 'Pipe 1', Date.now(), Date.now(), 'Nothis really', 3, null, true, null, null, 0),
      new Pipeline(2, 'lowkey', 'Pipe2', Date.now(), Date.now(), null, null, null, true, null, null, 0)
    ]
    ...
    const pipelineLead = new PipelineLead(2, 3, 'sdsdsdsd', 'My Pipe', null, 1, 3)
    ...
    const testPoint =
      new TestPoint(3, '787hdhujk-2ud-4hy-kski', 'MyTP1', 0, Date.now(), Date.now(), 'Nothing to say here', 'In your ass', 39.92862665, -12.37377837, 1)
    testPoint.setSubitems([
      ...
      new Coupon(4, 3, 'dsdsdss', 'Coupon 2', 2, 3, 2, 0, 23, null, 100, null),
      ...
      new TestLead(8, 3, 'dksldks', 'Test lead 1', 0, 3),
    ...
    const rectifier = new Rectifier(1, '787hdhujk-2ud-4hy-kski', 'hooks', 0, Date.now(), Date.now(), 'Very cool rectifier', 'Cant believe it', 39.92862665, -12.37377837, 'My basic', 'non34urbusiness', 0, null, null, 2, 43, 2, 4, null, 34)
    rectifier.setSubitems([
      new Circuit(1, 1, 'dasas', 'JKdjdj', null, null, 1, 2, 1.5, 2, null),
      new AnodeBed(1, 1, '2323', 'Bed', 0, 0, 0, [
        new AnodeBedAnode(1, 'asa', 1, 2.2, null, null),
    ...
    try {
      const link = encoder.encode(rectifier, pipelines, referenceCells, potentialTypes)
