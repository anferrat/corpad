...
import { initBluetooth, testMultimeter } from '../app/controllers/MultimeterController'
...
export default DevScreen = ({ navigation, route }) => {
  ...
  const testBluetooth = async () => {
    const { response, status, errorMessage } = await testMultimeter()
    console.log(response, status, errorMessage)
  }

  const initBluetoothModule = async () => {
    const { status, response, errorMessage } = await initBluetooth()
    console.log(response, status, errorMessage)
  }
  ...
  return (
    <SafeAreaView style={{ ...globalStyle.screen, paddingTop: StatusBar.currentHeight }}>
      ...
      <Button onPress={initBluetoothModule} appearance='ghost'>INIT Bluetooth</Button>
      <Button onPress={testBluetooth} appearance='ghost'>Test Bluetooth</Button>
