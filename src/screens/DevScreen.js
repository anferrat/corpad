...
import { addVoltageListener, requestVoltage } from '../app/controllers/MultimeterController'
...
export default DevScreen = ({ navigation, route }) => {
  ...
  const bufferTest = async () => {
    try {
      //const buffer = Buffer.from([0x1, 0xFF, 0x00, 0x00, 0x05, 0x00])
      const { status, errorMessage, response } = await requestVoltage()
    }
    catch (er) {
  ...
  useEffect(() => {
    ...
    const addListener = async () => {
      listener = await addVoltageListener()
    }
    addListener()
    return () => {
  ...
  return (
    <SafeAreaView style={{ ...globalStyle.screen, paddingTop: StatusBar.currentHeight }}>
      ...
      <Button onPress={bufferTest} appearance='ghost'>TEST Buffer</Button>
    </SafeAreaView>
  )
}
