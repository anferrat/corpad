import { ADD_ITEM_TO_LIST, LOAD_LIST_STATE, UPDATE_LIST, DELETE_ITEM_FROM_LIST, SET_UPDATING, SET_RESFRESH, SET_DISPLAYED_READING, SET_OFFSET, RESET_LIST_STATE } from '../actions/pipelineList'
    ...
    switch (action.type) {
        case LOAD_LIST_STATE:
            {
                return {
                    ...
                    itemList: state.settings.idListLoaded ? [...state.itemList, ...action.itemList] : [...action.itemList],
                    settings: {
                        ...state.settings,
                        endReached: (action.itemList.length < state.settings.limit) || (action.itemList.length + state.itemList.length === state.idList.length),
                        idListLoaded: true,
                        refreshing: false
                    },
                }
        ...
        case ADD_ITEM_TO_LIST:
            return {
                ...
                idList: [action.itemId].concat(state.idList),
                settings:
                {
                    ...state.settings,
                    updating: null
