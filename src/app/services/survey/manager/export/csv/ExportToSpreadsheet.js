...
export class ExportToSpreadsheet {
    constructor(surveyRepo, testPointRepo, rectifierRepo, pipelineRepo, potentialRepo, potentialTypeRepo, fileSystemRepo, csvParser, fileNameGenerator, assetRepo) {
    ...
    async execute({ itemType, sorting, itemProperties, exportPotentials, referenceCellId, potentialTypeIdList, selectedSubitemTypes, pipelineIdList, groupPotentialsByPipeline, subitemProperties, includeAssets }) {
        ...
        const writtenFile = await this.fileSystemRepo.writeFile(fileContent, `${fileName}.csv`, FileSystemLocations.EXPORTS, false)
        if (includeAssets)
