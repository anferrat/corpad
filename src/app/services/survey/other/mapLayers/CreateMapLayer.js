...
export class CreateMapLayer {
    constructor(mapLayerRepo, mapLayerPresenter, geoJsonValidation, geoJsonParser, geoJsonPointExtractor) {
    ...
    async execute({ name, width, color, data, comment, defaultName }) {
        ...
        const { geoJson, points } = this.geoJsonPointExtractor.execute(validGeoObject)
