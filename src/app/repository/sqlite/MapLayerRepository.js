...
export class MapLayerRepository extends SQLiteRepository {
    ...
    async getById(id) {
        try {
            ...
            return new MapLayer(id, uid, name, comment, timeCreated, timeModified, strokeColor, strokeWidth, fillColor, data, visible)
        }
        catch (er) {
    ...
    async update(mapLayer) {
        try {
            ...
            await this.runSingleQueryTransaction('UPDATE mapLayers SET uid=?, name=?, comment=?, timeCreated=?, timeModified=?, strokeColor=?, strokeWidth=?, fillColor=?, data=?, visible=? WHERE id=?', [uid, name, comment, timeCreated, timeModified, strokeColor, strokeWidth, fillColor, data, visible, id])
            return mapLayer
        }
        catch (er) {
    ...
    async create(mapLayer) {
        try {
            const { id, uid, name, comment, timeCreated, timeModified, strokeColor, strokeWidth, fillColor, data, visible } = mapLayer
            await this.runSingleQueryTransaction('INSERT INTO mapLayers (id, uid, name, comment, timeCreated, timeModified, strokeColor, strokeWidth, fillColor, data, visible) VALUES (?,?,?,?,?,?,?,?,?,?,?)', [id, uid, name, comment, timeCreated, timeModified, strokeColor, strokeWidth, fillColor, data, visible])
            return mapLayer
        }
        catch (er) {
    ...
    async delete(id) {
        try {
            await this.runSingleQueryTransaction('DELETE FROM mapLayers WHERE id=?'), [id]
        }
        catch (er) {
